...
from demucs.evaluate import evaluate, new_sdr
...
class KTSolver(object):
    def __init__(self, loaders, student_model, teacher_model, optimizer, args):
        self.args = args
        ...
        self.quantizer = states.get_quantizer(self.student_model, args.quant, self.optimizer)
        self.dmodel = distrib.wrap(student_model)
        self.device = next(iter(self.student_model.parameters())).device

        # Exponential moving average of the student_model, either updated every batch or epoch.
        # The best student_model from all the EMAs and the original one is kept based on the valid
        # loss for the final best student_model.
        ...
        for kind in self.emas.keys():
            ...
            if decays:
                for decay in decays:
                    self.emas[kind].append(ModelEMA(self.student_model, decay, device=device))
    ...
    def _serialize(self, epoch):
        package = {}
        package['state'] = self.student_model.state_dict()
        ...
        if self.best_changed:
            # Saving only the latest best student_model.
            with write_and_rename(self.best_file) as tmp:
                package = states.serialize_model(self.student_model, self.args)
    ...
    def _reset(self):
        ...
        if self.checkpoint_file.exists():
            logger.info(f'Loading checkpoint student_model: {self.checkpoint_file}')
            package = torch.load(self.checkpoint_file, 'cpu')
            self.student_model.load_state_dict(package['state'])
            ...
            student_model = pretrained.get_model(
                name=self.args.continue_pretrained,
                repo=self.args.pretrained_repo)
            self.student_model.load_state_dict(student_model.state_dict())
            ...
            if self.args.continue_best:
                self.student_model.load_state_dict(package['best_state'], strict=False)
            else:
                self.student_model.load_state_dict(package['state'], strict=False)
    ...
    def _format_test(self, metrics: dict) -> dict:
        ...
        if 'nsdr' in metrics:
            losses['nsdr'] = format(metrics['nsdr'], '.3f')
        for source in self.student_model.sources:
            key = f'sdr_{source}'
            if key in metrics:
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.args.epochs):
            ...
            self.student_model.eval()  # Turn off Batchnorm & Dropout
            with torch.no_grad():
                valid = self._run_one_epoch(epoch, train=False)
                bvalid = valid
                bname = 'main'
                state = states.copy_state(self.student_model.state_dict())
                metrics['valid'] = {}
            ...
            if self.args.svd.penalty > 0:
                ...
                kw.pop('penalty')
                with torch.no_grad():
                    penalty = svd_penalty(self.student_model, exact=True, **kw)
            ...
            # Eval student_model every `test.every` epoch or on last epoch
            ...
            if should_eval or is_last:
                ...
                # We switch to the best known student_model for testing
    ...
    def _run_one_epoch(self, epoch, train=True):
        ...
        for idx, sources in enumerate(logprog):
            ...
            if not train and self.args.valid_apply:
                estimate = apply_model(self.student_model, mix, split=self.args.test.split, overlap=0)
            else:
                estimate = self.dmodel(mix)
            if train and hasattr(self.student_model, 'transform_target'):
                sources = self.student_model.transform_target(mix, sources)
            ...
            if not train:
                ...
                total = 0
                for source, nsdr, w in zip(self.student_model.sources, nsdrs, weights):
            ...
            if train and args.svd.penalty > 0:
                kw = dict(args.svd)
                kw.pop('penalty')
                penalty = svd_penalty(self.student_model, **kw)
                losses['penalty'] = penalty
                loss += args.svd.penalty * penalty

            losses['loss'] = loss

            for k, source in enumerate(self.student_model.sources):
                losses[f'reco_{source}'] = reco[k]

            # optimize student_model in training mode
            if train:
                loss.backward()
                grad_norm = 0
                grads = []
